    ...
    <div className="flex-1 w-full h-full bg-transparent flex flex-col items-center justify-start overflow-auto relative py-8 px-4">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs font-mono font-black uppercase tracking-[0.3em] text-indigo-400/90">
            Doubly Linked List — Capacity {CAPACITY}
          </span>
          <div className="w-2 h-2 rounded-full bg-indigo-400 shadow-[0_0_8px_rgba(99,102,241,0.8)] animate-pulse" />
        </div>
              ...
              {listItems.map((val, idx) => {
                ...
                return (
                  <React.Fragment key={idx}>
                    ...
                    >
                      {/* Left: Prev Link Node Pin */}
                      <div className="px-2 py-2.5 flex flex-col items-center justify-center bg-slate-950/40 min-w-7 border-r border-slate-800/20">
                        <span className="text-[8px] font-mono text-slate-600">prev</span>
                        <div className="w-2 h-2 rounded bg-indigo-500/50 flex items-center justify-center">
                          <div className="w-1 h-1 rounded bg-indigo-300" />
                        </div>
                      </div>
                      ...
                      {/* Right: Next Link Node Pin */}
                      <div className="px-2 py-2.5 flex flex-col items-center justify-center bg-slate-950/40 min-w-7">
                        <span className="text-[8px] font-mono text-slate-600">next</span>
                        <div className="w-2 h-2 rounded bg-emerald-500/50 flex items-center justify-center">
                          <div className="w-1 h-1 rounded bg-emerald-300" />
                        </div>
                      </div>
                    </motion.div>

                    {/* Bidirectional Link Arrows */}
                    <div className="flex items-center text-indigo-500/60 px-0.5">
        ...
        <div className="flex items-center gap-4 px-5 py-2.5 rounded-xl bg-slate-950/80 border border-slate-800/60 text-[10px] font-mono">
          <div className="flex items-center gap-1.5">
            <span className={`w-1.5 h-1.5 rounded-full ${isEmpty ? 'bg-slate-500' : isFull ? 'bg-red-500' : 'bg-indigo-500'}`} />
