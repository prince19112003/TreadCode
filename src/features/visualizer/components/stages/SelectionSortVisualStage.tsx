...
import { Sparkles, ArrowDown, Lock, CheckCircle2 } from 'lucide-react';

export const SelectionSortVisualStage: React.FC = () => {
  ...
  if (memory.arr) {
    try {
      ...
      console.error('Error parsing array in SelectionSortVisualStage:', e);
    }
  }
  ...
  const minIdx = typeof memory.minIdx === 'number' ? memory.minIdx : -1;

  // Determine step action state
  const isSwapStep = explanation.toLowerCase().includes('swap');
  const isNewMinStep = explanation.toLowerCase().includes('new min') || explanation.toLowerCase().includes('minidx');
  const isCompareStep = explanation.toLowerCase().includes('compare') || currentJ >= 0;
  const isSortedStep = explanation.toLowerCase().includes('sorted') || currentStepIndex === (activeSteps?.length || 1) - 1;
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-between p-6 bg-slate-950/60 rounded-3xl border border-slate-800/80 backdrop-blur-xl relative overflow-hidden select-none">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-purple-500/10 blur-[100px] pointer-events-none rounded-full" />
      <div className="absolute bottom-10 right-10 w-72 h-72 bg-cyan-500/10 blur-[90px] pointer-events-none rounded-full" />

      {/* Header Info & State Badge */}
      <div className="w-full flex items-center justify-between z-10">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-purple-500/10 border border-purple-500/30 flex items-center justify-center text-purple-400 font-bold shadow-lg shadow-purple-500/10">
            <Sparkles size={20} />
          </div>
          <div>
            <h3 className="text-white font-black tracking-wide text-base">SELECTION SORT VISUALIZER</h3>
            <p className="text-xs font-mono text-slate-400">O(N²) Time Complexity • Min Element Selection</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {isSortedStep ? (
            ...
            <div className="px-3.5 py-1.5 rounded-full bg-rose-500/15 border border-rose-500/40 text-rose-300 font-mono font-bold text-xs flex items-center gap-1.5 shadow-lg shadow-rose-500/20 animate-bounce">
              🔄 SWAPPING arr[{currentI}] ↔ arr[{minIdx}]
            </div>
          ) : isNewMinStep ? (
            <div className="px-3.5 py-1.5 rounded-full bg-cyan-500/15 border border-cyan-500/40 text-cyan-300 font-mono font-bold text-xs flex items-center gap-1.5 shadow-lg shadow-cyan-500/20">
              ✨ NEW MIN FOUND: minIdx = {minIdx}
            </div>
          ) : isCompareStep ? (
            <div className="px-3.5 py-1.5 rounded-full bg-amber-500/15 border border-amber-500/40 text-amber-300 font-mono font-bold text-xs flex items-center gap-1.5 shadow-lg shadow-amber-500/20">
              🔍 COMPARING arr[{currentJ}] & arr[{minIdx}]
            </div>
          ) : (
      ...
      <div className="w-full flex-1 flex items-end justify-center gap-4 md:gap-6 my-6 z-10 px-4">
        <AnimatePresence mode="popLayout">
          {arr.map((val, idx) => {
            const isTargetBoundary = idx === currentI && !isSortedStep;
            const isMin = idx === minIdx && !isSortedStep;
            const isScanner = idx === currentJ && !isSortedStep;
            const isSorted = isSortedStep || (currentI >= 0 && idx < currentI);
            const isSwapping = isSwapStep && (idx === currentI || idx === minIdx);
            ...
            if (isSorted) {
              ...
              bgClass = 'bg-linear-to-t from-cyan-950 via-cyan-800 to-cyan-500 border-cyan-400 text-white';
            ...
            } else if (isTargetBoundary) {
              bgClass = 'bg-linear-to-t from-purple-950 via-purple-800 to-purple-500 border-purple-400 text-white';
              glowShadow = 'shadow-[0_0_25px_rgba(168,85,247,0.3)]';
            }
      ...
      <div className="w-full bg-slate-900/90 border border-slate-800 rounded-2xl p-3 flex items-center justify-between text-xs font-mono z-10">
        ...
        <span className="text-slate-100 font-bold truncate max-w-[80%]">
          {explanation || 'Select or enter array values to visualize Selection Sort step-by-step.'}
        </span>
      </div>
    </div>
  );
