        ...
        <div className="flex flex-col items-center gap-1.5 select-none text-center relative">
          {isDsa ? (
            <button
              ...
              className={`w-9 h-9 rounded-lg flex flex-col items-center justify-center gap-0.5 transition-colors cursor-pointer border select-none ${
                showCheatSheet
                  ? 'bg-amber-500/20 border-amber-500/50 text-amber-300'
                  : 'bg-slate-800/40 hover:bg-slate-800/70 border-slate-700/60 text-slate-400 hover:text-slate-200'
              }`}
              title="Toggle Notes & Cheat Sheet"
            >
              <BookOpen size={14} className={showCheatSheet ? 'text-amber-400' : 'text-slate-400'} />
              <span className={`text-[7px] font-bold tracking-wider uppercase leading-none ${showCheatSheet ? 'text-amber-300' : 'text-slate-400'}`}>NOTES</span>
            </button>
          ) : (
